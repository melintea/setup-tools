...
//
// TURN server script. Uses https://github.com/pion/turn/releases
// Run at a cygwin prompt: ./server-script.go
//
...
func (s *stunLogger) WriteTo(p []byte, addr net.Addr) (n int, err error) {
    ...
    //[ Add an atttribute
	if err = stun.NewSoftware("CustomPionTurnServer").AddTo(msg); err != nil {
        log.Println(err)
		return
	}
...
func main() {
    publicIP := flag.String("public-ip", "10.254.64.207", "IP Address that TURN can be contacted by.")
    ...
    log.Println("Server listening on ", *publicIP, ":", *port)
